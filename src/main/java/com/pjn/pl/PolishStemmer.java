...
final class PolishStemmer implements IStemmer, Iterable<WordData> {
	/**
				...
				try {
					dictionary = AccessController.doPrivileged(new PrivilegedExceptionAction<Dictionary>() {
						...
						public Dictionary run() throws Exception {
							URL dictResource = new File("src/main/resources/polish.dict").toURI().toURL(); // getClass().getResource("polish.dict");
